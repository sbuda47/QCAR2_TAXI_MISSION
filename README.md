...
1. Traffic Detector (traffic_detector)
   * Runs dual YOLO inference:
       * YOLOv8 COCO for stop sign
       * Pretrained traffic-light model for red/yellow/green/off
2. Taxi Planner (taxi_planner)
   * Simple finite-state mission planner that publishes the current target waypoint.
   * Publishes:
       * /taxi_target (PoseStamped)
       * /taxi_state (String): e.g., GO_PICK, GO_DROP, RETURN_HUB
3. Main Controller (main_controller) — the only actuator authority
...
1. Quanser Virtual QCar2 Docker container (spawns QLabs map + assets)
2. Isaac ROS dev container (runs ROS 2 stack and our nodes)
